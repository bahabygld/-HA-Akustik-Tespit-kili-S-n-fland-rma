%% ===================== ÖZELLİK ÇIKARIMI - VAL (BALANCED_V3_NOISE) =====================
clear; clc;

root      = "C:\Users\baha_\OneDrive\Masaüstü\BALANCED_V3_NOISE";
valPosDir = fullfile(root, "val", "pos");
valNegDir = fullfile(root, "val", "neg");

fprintf("Val POS klasörü: %s\n", valPosDir);
fprintf("Val NEG klasörü: %s\n", valNegDir);

srTarget = 16000;
winDur   = 0.025;
hopDur   = 0.010;
NUM_MFCC = 13;
NUM_MEL  = 64;

Tpos_va = extract_features_from_folder(valPosDir, 'pos', srTarget, NUM_MFCC, NUM_MEL, winDur, hopDur);
Tneg_va = extract_features_from_folder(valNegDir, 'neg', srTarget, NUM_MFCC, NUM_MEL, winDur, hopDur);

T_val = [Tpos_va; Tneg_va];
fprintf("VAL toplam örnek: %d (pos=%d, neg=%d)\n", ...
    height(T_val), sum(T_val.LabelCat=="pos"), sum(T_val.LabelCat=="neg"));

% ---- CL için kolon seçimi (F_* + LabelCat) ----
fcols    = startsWith(T_val.Properties.VariableNames, "F_");
idxLabel = strcmp(T_val.Properties.VariableNames, "LabelCat");
colIdx   = fcols | idxLabel;

T_val_cl = T_val(:, colIdx);

save(fullfile(root, "T_val.mat"), "T_val");
save(fullfile(root, "T_val_cl.mat"), "T_val_cl");

fprintf("Kaydedildi: %s\n", fullfile(root, "T_val.mat"));
fprintf("Kaydedildi: %s\n", fullfile(root, "T_val_cl.mat"));

return;

%% ============================ LOCAL FUNCTION =============================

function T = extract_features_from_folder(folderPath, labelStr, srTarget, NUM_MFCC, NUM_MEL, winDur, hopDur)
    ads = audioDatastore(folderPath, 'IncludeSubfolders', false, 'FileExtensions', '.wav');
    n   = numel(ads.Files);
    fprintf("\n[%s] klasöründe %d dosya bulundu.\n", labelStr, n);

    feats  = cell(n,1);
    labels = cell(n,1);
    files  = strings(n,1);

    for i = 1:n
        src = ads.Files{i};
        try
            [x, fs] = audioread(src);
            if size(x,2)>1, x = mean(x,2); end
            if fs ~= srTarget
                x = resample(x, srTarget, fs);
                fs = srTarget;
            end
            x = unit_peak(x);

            feat = extract_157d(x, fs, NUM_MFCC, NUM_MEL, winDur, hopDur);

            feats{i}  = single(feat);
            labels{i} = labelStr;
            files(i)  = string(src);
        catch ME
            warning("Atlandı: %s -> %s", src, ME.message);
            feats{i}  = nan(1,157,'single');
            labels{i} = labelStr;
            files(i)  = string(src);
        end

        if mod(i, max(1,floor(n/10)))==0
            fprintf("[%s] %d/%d\n", labelStr, i, n);
        end
    end

    X = vertcat(feats{:});
    bad = any(~isfinite(X),2);
    if any(bad)
        warning("[%s] NaN/Inf içeren %d örnek atıldı.", labelStr, nnz(bad));
        X(bad,:)     = [];
        files(bad)   = [];
        labels(bad)  = [];
    end

    fn    = "F_" + (1:size(X,2));
    T     = array2table(X, 'VariableNames', fn);
    label = categorical(labels(:), {'neg','pos'}, 'Ordinal', false);
    if labelStr=="pos"
        label(:) = categorical("pos", {'neg','pos'});
    else
        label(:) = categorical("neg", {'neg','pos'});
    end
    T.LabelCat = label;
    T.File     = files;
end

function x = unit_peak(x)
    pk = max(abs(x)); if pk>0, x = x/pk; end
end

function feat = extract_157d(x, fs, NUM_MFCC, NUM_MEL, winDur, hopDur)
    C = mfcc(x, fs, 'NumCoeffs', NUM_MFCC);
    if size(C,1)<size(C,2), C = C.'; end
    mf_m = mean(C,1);
    mf_s = std(C,0,1);

    wl  = max(32, round(winDur*fs));
    hl  = max(16, round(hopDur*fs));
    win = hamming(wl, 'periodic');
    S   = melSpectrogram(x, fs, 'Window', win, ...
        'OverlapLength', wl-hl, 'NumBands', NUM_MEL, 'FFTLength', 1024);
    Slog = log10(S + 1e-12);
    ml_m = mean(Slog,2).';
    ml_s = std(Slog,0,2).';

    zcr = mean(zerocrossrate(x));
    rv  = rms(x);
    sc  = mean(spectralCentroid(x,fs));

    feat = [mf_m, mf_s, ml_m, ml_s, zcr, rv, sc];
end
